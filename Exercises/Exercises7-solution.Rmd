...
Eigenvalues can be calculated easily: $\lambda_1 = i\sqrt{ac}, \lambda_2 = -i\sqrt{ac}$. As shown by the eigenvalues which both have a zero real par. This indicates that the equiblirum is not attriactive and is not repulsive.S

Due to the fact the the eigenvalues have a non-zero imaginary part, the system will now oscilate awith a peried of $\sqrt{ac}$
...
(1) and (2) is trivial since transition can be made from any state to another.
...
The expected payoff at the stationary distribution of the correspounding Markov chain is the same for both players.

...
Since $f_0 > 1$ and $0 < q < 1$, the matrix W is non-negative. Besides, the shape of W is $2 \times 2, (I + W)^{n-1} = I + W > 0$, so the matrix W is irreducible. We can apply the Perron–Frobenius theorem. 

According to this theorem, $\lambda_{max}$ is simple and the components of the associated eigenvector are all (strictly) positive. Therefore, the largest eigenvalue $\lambda = \frac{q(f_0+1) + \sqrt{q^2(f_0+1)^2  - 4f_0(2q-1)} }{2}$ corresponds the non-trivial equilibrium point.
...
$$
\lambda_{max} = \frac{2q + \sqrt{4(q-1)^2} }{2} = 1
$$

So $\phi = \lambda_{max} = x_0 + x_1 = 1$
...
By b) we know that the equilibrium point for $q \approx 1$ would be 

$$
\lambda_{max} = \frac{f_0 + 1 + \sqrt{(f_0-1)^2} }{2} = f_0
$$

So $\phi = \lambda_{max} = f_0x_0 + x_1 = f_0$
